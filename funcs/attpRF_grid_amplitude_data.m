function visual_field_map = attpRF_grid_amplitude_data(currROI, path2project, subject, session, designFolder, x_grid, y_grid, masked_grid, rotation)
...
prfFolder = fullfile(path2project, 'derivatives', 'prfs', sprintf('%s',subject),...
    sprintf('ses-%s',session), 'prfFolder_2', 'avg/');
...
for conds = 1:length(conditions)
    ...
    %betas = thresholded_betas(indices,conds)-thresholded_betas(indices,5);
