...
params      = [mxecc,0,0,1,3,sigma,spacegrid];
[stimdrive_base, numeratorIm_base, suppIm_base, baselineSpatialResponse, baselinepredneuralweights, baselinepredsummedweights] ...
...
for cond = 1:length(attx0locs)
    params      = [mxecc,attgain,attx0locs(cond),atty0locs(cond),attsd,sigma,spacegrid];
    [stimdriveIm(:,:,:,cond), numeratorIm(:,:,:,cond), suppIm(:,:,:,cond), sptPopResp(:,:,:,cond), predneuralweights(:,:,cond), predsummedweights(:,:,cond)] ...
        = NMA_simulate2D(maindir, params);
end

for ii = 1:length(attx0locs)
    predsummedweightsIm(:,:,:,ii) = reshape(predsummedweights(:,:,ii), 64, 64, []);
end
...
for condind = 1:length(attx0locs)
    ...
    axis off
    subplot(1,4,4)
    imagesc(sum(predsummedweightsIm(:,:,:,condind),3),clims)
    title('Population response')
    colormap(cmap)
    set(gcf, 'Position', [0 0 850 220])
    set(gcf,'color','white')
