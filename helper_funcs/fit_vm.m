    ...
    switch fit_func
        case 'von_mises'
            if isnan(curr_Y(1))
                ...
                f = fittype('a*exp(kappa*cos(deg2rad(x)-mu)-log(2*pi*besseli(0,kappa))) + baseline','independent', 'x', 'coefficients', {'a', 'kappa', 'baseline', 'mu'});
        ...
        case 'diff_von_mises'
            if isnan(curr_Y(1))
                ...
                f = fittype('a1*(exp(kappa1*cos(deg2rad(x)-mu)-log(2*pi*besseli(0,kappa1)))) - a2 * (exp(kappa2*cos(deg2rad(x)-mu)-log(2*pi*besseli(0,kappa2)))) + baseline', ...
                    'independent', 'x', 'coefficients', {'a1','a2', 'kappa1','kappa2', 'baseline','mu'});
               
                fo = fitoptions('Normalize','off',...
                    'MaxFunEvals',1500, ...
                    'MaxIter',1500, 'Method', 'NonlinearLeastSquares', 'StartPoint', [1, 1, 0, 0, 0, 0]);
                [fitresult, gof] = fit(X', curr_Y', f, fo);
                rsquare(1,roi) = gof.rsquare;
